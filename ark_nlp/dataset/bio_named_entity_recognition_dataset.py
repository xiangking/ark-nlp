...
class BIONERDataset(TokenClassificationDataset):
    ...
    def _get_categories(self):
        ...
        categories = []
        types = set([label['type'] for data in self.dataset for label in data['label']])
        for type in types:
            categories.append('B-' + type)
        ...
        for index, row in enumerate(
            ...
            if not self.is_test:
                ...
                for info in row['label']:
                    if info['start_idx'] in start_mapping and info[
                            'end_idx'] in end_mapping:
                        start_idx = start_mapping[info['start_idx']]
                        end_idx = end_mapping[info['end_idx']]
                        if start_idx > end_idx or info['entity'] == '':
                            continue

                        label_ids[start_idx + 1] = self.cat2id['B-' + info['type']]

                        label_ids[start_idx + 2:end_idx + 2] = [
                            self.cat2id['I-' + info['type']]] * (end_idx - start_idx)
